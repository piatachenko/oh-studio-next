    ...
    <>
      {!notLink ? (
        ...
        >
          {video ? (
            <video
              ...
              muted
              loop
              className={`linkImages min-h-full min-w-full object-cover scale-[1.1] transition-all duration-300 ${customClass}`}
            />
          ) : (
            <img
              src={src}
          ...
          <span className="linkBlurs absolute inset-0 transition-all duration-300 block max-md:hidden">
        ...
        <div className="bigLinkItems relative flex-1 overflow-clip rounded-xl">
          ...
          <span className="md:hidden absolute top-5 right-5 rounded-full w-12 h-12 bg-white p-[0.9rem]">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
      ...
      {!notLink ? (
        <p className="md:hidden ml-1">{text}</p>
      ) : blocked ? (
        <p className="md:hidden ml-1">
          {text}{" "}
          <span className="text-[#838383] text-sm ml-0.5">Coming Soon</span>
        </p>
      ) : null}
    </>
  );
